...
class omnific_script:
    ...
    def reset_ball(self):
        player_x_direction = omnific.get_random_boolean()
        self.ball_physics_body.linear_velocity.x = omnific.get_random_float(12.0, 14.0) * (1.0 if player_x_direction else -1.0)
        self.ball_physics_body.linear_velocity.y = omnific.get_random_float(5.0, 8.0)
    ...
    def on_fixed_update(self):
        ...
        if omnific.has_event(constants.game_is_playing_event):
            ...
            if self.ball_transform.translation.x < self.left_post_transform.translation.x or self.ball_transform.translation.x > self.right_post_transform.translation.x:
                ball_speed = omnific.sqrt(self.ball_physics_body.linear_velocity.x * self.ball_physics_body.linear_velocity.x + 
            ...
            (omnific.has_continuous_event("entity_is_colliding", "Ball_and_Paddle2") and self.ball_physics_body.linear_velocity.x > 0.0)):
                ...
                self.ball_physics_body.linear_velocity.y = omnific.get_random_float(-1.0, 1.0) * omnific.get_random_float(3.0, 30.0)
