...
class omnific_script:
    ...
    def on_entity_start(self):
        ...
        self.start_message_gui = scene.get_component_from_entity_by_name("GUI", "Start Message")
        self.play_pause_button_gui = scene.get_component_from_entity_by_name("GUI", "Play/Pause Button")
        self.previous_button_gui = scene.get_component_from_entity_by_name("GUI", "Previous Button")
        self.next_button_gui = scene.get_component_from_entity_by_name("GUI", "Next Button")
    ...
    def on_update(self):
        ...
        if active_audio_clip is not None:
            ...
            self.oscilloscope_gui.set_to_plot(self.audio_source.get_current_waveform(), omnific.Colour(0, 0, 0), omnific.Colour(255, 255, 255))
            self.start_message_gui.hide()
        if self.play_pause_button_gui.get_root_element().is_clicked:
    ...
    def on_late_update(self):
        ...
        if len(file_drop_events) > 0:
            ...
            if omnific.get_file_access().exists(audio_filepath):
                self.start_message_gui.set_to_label("Loading: " + audio_filepath)
